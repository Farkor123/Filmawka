...
begin
	declare main_review_id int;
    select m.review_id from main_reviews m where m.tv_series_id=tv_series_id into main_review_id;
    return main_review_id;
end//
DELIMITER ;
...
DELIMITER //
create function get_the_most_pointed_review_by_movie_id(movie_id int) returns int reads sql data
begin
...
begin
	declare review_id int;
    select w.review_id from waiting_reviews w where w.tv_series_id=tv_series_id AND w.points=(select MAX(ww.points) 
		from waiting_reviews ww where ww.tv_series_id=tv_series_id) into review_id;
    return review_id;
end//
DELIMITER ;
...
begin
    ...
    declare title, original_title varchar(30);
     if exists(select * from tv_series t where t.tv_series_id = tv_series_id) then
        ...
        select *,title,original_title from waiting_reviews w where w.tv_series_id=tv_series_id;
...
begin
		...
		set is_movie=true;
			select w.movie_id from waiting_reviews w where w.review_id=review_id into tv_id;
        if (select tv_id) IS NULL then
			select w.tv_series_id from waiting_reviews w where w.review_id=review_id into tv_id;
            SET is_movie=false;
		end if;
		if(is_movie) then
			select m.title, m.original_title
...
begin
    ...
    then
		select m.movie_id from main_reviews m where m.review_id=review_id into tv_id;
		if(tv_id) is null
		then
			select m.tv_series_id from main_reviews m where m.review_id=review_id into tv_id;
            SET is_movie=false;
		end if;
		...
		END IF;
			...
			select m.review_id,m.old_review_id,txt as 'film/seria tv',tv_id as 'id filmu/serii tv',title,original_title,w.review_writer_id,first_name_writer,last_name_writer, m.accept_review_moderator 
...
begin
    ...
    if (select is_moderator from users where user_id=moderator_id)=1 then
		if exists(select * from waiting_reviews w where w.review_id=waiting_review_id)
			then
				SET tv_id=(select w.movie_id from waiting_reviews w where w.review_id= waiting_review_id LIMIT 1);
                if (tv_id) IS NOT NULL
                then
					SET tv_id=(select w.tv_series_id from waiting_reviews w where w.review_id=waiting_review_id LIMIT 1);
					set is_movie=false;
                end if;
				SET new_review=(select w.review from waiting_reviews w where w.review_id=waiting_review_id);
			if exists(select * from main_reviews m where m.movie_id=tv_id) OR exists (select * from main_reviews m where m.tv_series_id=tv_id) 
			then
                if (is_movie) then
					UPDATE main_reviews m SET m.review=new_review,m.accept_review_moderator= moderator_id,m.old_review_id=waiting_review_id where m.movie_id=tv_id;
				else
					UPDATE main_reviews m SET m.review=new_review,m.accept_review_moderator= moderator_id,m.old_review_id=waiting_review_id where m.tv_series_id=tv_id;
				end if;
			else
				if (is_movie) then
					insert into main_reviews(old_review_id,accept_review_moderator,movie_id,review) values (waiting_review_id,moderator_id,tv_id,new_review);
				else 
					insert into main_reviews(old_review_id,accept_review_moderator,tv_series_id,review) values (waiting_review_id,moderator_id,tv_id,new_review);
				end if;
		end if;
		else
...
begin
	select w.review_id from waiting_reviews w where w.tv_series_id=tv_series_id ;
end//
DELIMITER ;
...
-- CALL get_waiting_reviews_info_by_review_id(10);
--select * from main_reviews;
-- CALL get_main_review_info_by_review_id(4);
-- check : set_waiting_review_to_main_review,get_waiting_reviews_info_by_review_id