...
begin
    ...
    select w.review_id from waiting_reviews_movies w where w.movie_id=movie_id AND w.points=(select MAX(ww.points) 
		from waiting_reviews_movies ww where ww.movie_id=movie_id) into review_id;
    return review_id;
end//
DELIMITER ;
...
begin
    ...
    select w.review_id from waiting_reviews_tv_series w where w.tv_series_id=tv_series_id AND w.points=(select MAX(ww.points) 
		from waiting_reviews_tv_series ww where ww.tv_series_id=tv_series_id) into review_id;
    return review_id;
end//
DELIMITER ;
...
begin
    ...
    then
		if exists(select * from waiting_reviews_movies where review_id=waiting_review_id)
			then
				if is_user_logged_in(user_id_that_add_point) != 0 AND  exists(select * from points_on_reviews_movie where user_id=user_id_that_add_point)
					then
...
begin
    ...
    then
		if exists(select * from waiting_reviews_tv_series where review_id=waiting_review_id)
			then
				if is_user_logged_in(user_id_that_add_point) != 0 AND  exists(select * from points_on_reviews_tv_series where user_id=user_id_that_add_point)
					then
...
begin
    ...
    then
		if exists(select * from movies m where m.movie_id = movie_id) then
            ...
            then
				insert into waiting_reviews_movies(movie_id, review_writer_id, review, points) values (movie_id, user_id, review,0);
...
begin
    ...
    then
		if exists(select * from tv_series m where m.tv_series_id = tv_series_id) then
            ...
            then
				insert into waiting_reviews_tv_series(tv_series_id, review_writer_id, review, points) values (tv_series_id, user_id, review,0);
				select 'pomyslnie dodano recenzje' as info;
			else
...
begin
    ...
    if (select is_moderator from users where user_id=moderator_id)=1 AND is_user_logged_in(moderator_id) != 0   then
		if exists(select * from waiting_reviews_movies w where w.review_id=waiting_review_id)
			...
			then
					UPDATE main_reviews_movie m SET m.review=new_review,m.accept_review_moderator= moderator_id,m.old_review_id=waiting_review_id where m.movie_id=tv_id;
			else
					insert into main_reviews_movie(old_review_id,accept_review_moderator,movie_id,review) values (waiting_review_id,moderator_id,tv_id,new_review);
			end if;
		else
...
begin
    ...
    if (select is_moderator from users where user_id=moderator_id)=1 AND is_user_logged_in(moderator_id) != 0  then
		if exists(select * from waiting_reviews_tv_series w where w.review_id=waiting_review_id)
			...
			then
				UPDATE main_reviews_tv_series m SET m.review=new_review,m.accept_review_moderator= moderator_id,m.old_review_id=waiting_review_id where m.tv_series_id=tv_id;
			else
				insert into main_reviews_tv_series(old_review_id,accept_review_moderator,tv_series_id,review) values (waiting_review_id,moderator_id,tv_id,new_review);
			end if;
		else
			select 'nie ma takiego czekajcego review';
        end if;
